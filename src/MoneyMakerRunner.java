...
public class MoneyMakerRunner extends JFrame {
    ...
    public BigDecimal netWorth = new BigDecimal(new BigInteger("0"), 2);
    public BigDecimal wps = new BigDecimal(new BigInteger("0"), 2);
    public int page = 0;
    ...
    private void updatePurchasables(){
        ...
        WORTH.setText("<html>Worth: <br>$" + netWorth + "<br>CPS: $" + wps + "<html>");
    ...
    private void setUpShop(int y, String fileName, GridBagConstraints c) {
        ...
        JLabel text = new JLabel("<html>Cost: $" + asset.cost + "<br>Refund: $"
                                        + asset.cost.divide(new BigDecimal("2"), 2)
    ...
    private static BufferedImage resize(BufferedImage icon, int width, int height) {
    ...
    private void addBuySell(GridBagConstraints constraints, Assets a){
        JPanel buttonPanel = new JPanel();
        buttonPanel.setLayout(new BorderLayout());
        JButton sell = new JButton("sell");
        sell.addActionListener(e -> {
            if(a.count.compareTo(new BigInteger("0")) > 0) {
                BigDecimal subtracted = wps.subtract(a.revenue);
    ...
    private void run() {
        ISACTIVE = true;
        while (true) {
            netWorth = netWorth.add(wps.compareTo(new BigDecimal("1000")) > 0 ? wps.divide(new BigDecimal("1000"), 2):new BigDecimal("1"));
            try {
                WORTH.setText("<html>Worth: $" + netWorth + "<br>CPS: $" + wps + "<html>");
            }
            catch(Exception ignored){
                Font font = WORTH.getFont();
                if(font.getSize() == 5){
                    WORTH.setText("<html>Worth: $infinity<br>CPS: $infinity<html>");
                }
                else {
                    WORTH.setFont(new Font(font.getName(), font.getStyle(), font.getSize() - 5));
                    WORTH.setText("<html>Worth: $" + netWorth + "<br>CPS: $" + wps + "<html>");
                }
            }

            try {
                Thread.sleep(1000 / wps.intValue());
